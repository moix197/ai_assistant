...
import { type ConnectFlow, IDENTITY_SCOPES } from "@hermes/google-auth";
import { CHANNEL_TELEGRAM } from "../agent/build-agent";

const USAGE_TEXT = "Usage: /connect google";
...
/**
 * `/connect google` — the only supported provider this phase. Anything else
 * (`/connect bogus`, bare `/connect`) is handled locally with a usage
 * message and never reaches `connectFlow.startConnect`, closing the
 * paid-fallthrough hole `boot.ts`'s dispatcher argument parsing fixes.
 * `/disconnect` and `/status` are Phase 3's, not this handler's.
 */
export function createConnectHandler(
  ...
  return async function handleConnect(message: InboundMessage, args: string): Promise<void> {
    const provider = args.trim().split(/\s+/)[0] ?? "";
