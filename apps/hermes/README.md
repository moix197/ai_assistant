...
6. `acquireInstanceLock()` — a Postgres advisory lock enforcing the
   ...
   `process.exit()` (see `packages/store/README.md`).
...
8. Build the Telegram adapter (`createTelegramClient` + `createTelegramPoller`)
   ...
   (`src/handlers/with-allowlist.ts`) — the single allowlist gate for every
   handler. The dispatcher (`src/boot.ts`) routes `/ping` and `/start` to
   their handlers and everything else to `echo` (`src/handlers/echo.ts`),
   which also rejects non-private chats and ignores edited messages.
9. `registerShutdown()` — registers the SIGTERM/SIGINT handler (see below).

## Handlers

- `echo.ts` — echoes back the message text, private chats only.
...
1. `channel.stop()` — flips the poller's `stopping` flag so no new
   ...
   ~8s (`DRAIN_TIMEOUT_MS`) so a stuck drain can't block the rest of
...
A hard-exit fallback timer (`HARD_EXIT_TIMEOUT_MS`, ~9s) forces
