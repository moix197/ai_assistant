...
| `@hermes/store` | Postgres pool, migration runner, repositories (`telegram_offset`, `llm_usage`), single-instance advisory lock. | `packages/store` | [telegram-long-polling-correctness](decisions/telegram-long-polling-correctness.md), [llm-cost-accounting](decisions/llm-cost-accounting.md), [test-database-isolation](decisions/test-database-isolation.md) |
...
| DB integration test lane | `pnpm test:db` → `packages/store/src/__tests__/db-env.ts` (URL resolution + guard); scratch DB named `*_test` | The suites `DELETE FROM llm_usage`, the table the ceiling spends against, so the guard throws on a `TEST_DATABASE_URL` equal to `DATABASE_URL` or not ending in `_test`. Runs serially (`--no-file-parallelism`) because the two `llm_usage` files race. [test-database-isolation](decisions/test-database-isolation.md) — read before adding a DB suite or changing the scripts. |
