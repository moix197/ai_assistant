...
| `@hermes/llm` | `LlmProvider` port + one OpenAI-compatible adapter over `fetch` (retries, backoff, timeout, typed errors), plus the per-model pricing table and cost resolution. The only package that talks HTTP to a model provider. | `packages/llm` | [d5-deepseek-primary-gemini-fallback](decisions/d5-deepseek-primary-gemini-fallback.md), [llm-cost-accounting](decisions/llm-cost-accounting.md), [architecture](architecture.md#dependency-direction) |
