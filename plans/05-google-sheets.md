...
| create | `packages/store/src/sheet-write-log-repo.ts` | `claim(pool, key, { channel, channelUserId, turnId, tool, canonicalArgs }): Promise<"claimed" \| { alreadyComplete: true; outcome } \| { alreadyPending: true }>` (an `INSERT ... ON CONFLICT (dedupe_key) DO NOTHING` matching `llm-dedupe-repo`'s idiom, then a read-back to distinguish "I claimed it" from "already complete" from "still pending"; unlike `llm_dedupe`, a still-`pending` row is **not** fail-open — it's the claim-to-complete crash window, genuinely indistinguishable from a same-key write still in flight, so the caller must surface it as an ambiguous hedge rather than proceeding to write again), `complete(pool, key, outcome): Promise<void>`, `release(pool, key): Promise<void>` (`DELETE ... WHERE dedupe_key = $1 AND status = 'pending'` — releases a claim only after a *provably-definitive* failure, i.e. `sheets-write.ts` catching a `SheetsApiError` off `appendValues`, so a same-turn retry isn't permanently blocked hedging over a write that never landed; a genuinely ambiguous failure never calls this, keeping the fail-safe "when in doubt, hedge" default) |
| modify | `packages/store/src/index.ts` | export the new functions (`claim`, `complete`, and `release`) |
...
| create | `packages/google-sheets/src/tools/sheets-write.ts` | `ToolSpec`: `name: "sheets_write"`, `requiresApproval: true`, `timeoutMs: 30_000`, args a **flat `z.object`** (`{ mode: z.enum(["append", "update"]), sheet, range, values, valueInputOption? }`) — **deviates from this row's original `z.discriminatedUnion("mode", [...])` spec, fixed post-manual-verification**: both branches had identical field sets (only the `mode` literal differed), so the union added no validation, but `z.toJSONSchema` (`packages/agent/src/prompt.ts`, the actual LLM-request conversion path) renders a discriminated union as a top-level `anyOf` with no `type: "object"` — which DeepSeek's OpenAI-compatible API rejects outright with HTTP 400, and since every tool schema is sent on every completion request, that one malformed schema broke *every* turn, not just writes. The flat object is behaviorally equivalent (same runtime validation, same append/update branching) and yields a top-level `type: "object"` schema; handler: resolve the slug (unknown ⇒ structured refusal, same as read tools), refuse before any API call if `entry.access !== "readwrite"` (`{ ok: false, reason: "read_only_sheet" }`), compute the dedupe key from `(ctx.channel, ctx.channelUserId, ctx.turnId, "sheets_write", canonicalArgs)`, `claim` — an already-complete claim returns the stored outcome without calling the Sheets API again, resolve `valueInputOption` (arg override, else the registry row's default), call `appendValues`/`updateValues`; on success `complete` with the written result and return it; for `mode: "update"`, the client already resolved any post-send ambiguity internally (see above), so this layer only ever sees success or a genuine fatal error; for `mode: "append"`, on the ambiguous-outcome class `complete` with an explicit "may or may not have landed" outcome and return that to the model without retrying; on a provably-definitive failure instead (a `SheetsApiError` — a non-429 4xx, or an exhausted 429 — meaning Google rejected the request without applying it), `release` the pending claim before rethrowing, so a same-turn retry isn't blocked hedging over a write that never landed |
| modify | `packages/google-sheets/src/index.ts` | export `createSheetsWriteTool` |
| modify | `packages/agent/src/index.ts` | **added post-manual-verification, not in original scope**: export `assemblePrefix`/`AssembledPrefix` from `./prompt` — needed so the new cross-tool schema regression test (below) can call the exact same zod→JSON-Schema conversion function the production LLM request path uses, rather than a re-implementation |
| create | `apps/hermes/src/agent/__tests__/tool-schemas.test.ts` | **added post-manual-verification, not in original scope**: regression test asserting every tool in `buildAgent`'s actual registered tool array converts (via the real `assemblePrefix`) to a JSON Schema with top-level `type: "object"` — this is the test that would have caught the `sheets_write` bug; unit tests passed while the bot was completely broken because nothing checked the wire-format shape of the full registered tool list |
